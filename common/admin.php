...
if ( !class_exists( 'MeowKit_MGL_Admin' ) ) {

  class MeowKit_MGL_Admin {
    ...
    public static function process_deferred_instances() {
      ...
      if ( !function_exists( 'current_user_can' ) || !function_exists( 'wp_get_current_user' ) ) {
        trigger_error(
          'MeowKit_MGL_Admin: Pluggable functions still unavailable on init hook. ' .
    ...
    private function run_admin_setup() {
      ...
      if ( !MeowKit_MGL_Admin::$loaded ) {
        // Check for potential issues with WordPress install, other plugins, etc.
        new MeowKit_MGL_Issues( $this->prefix, $this->mainfile, $this->domain );
      ...
      if ( $this->is_user_admin() ) {
        if ( !$this->disableReview ) {
          new MeowKit_MGL_Ratings( $this->prefix, $this->mainfile, $this->domain );
        }
        new MeowKit_MGL_News( $this->domain );
      }
    }
    ...
    public function meowapps_hide_dashboard_callback() {
      ...
      echo MeowKit_MGL_Helpers::wp_kses( $html );
    }
